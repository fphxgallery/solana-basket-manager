                        ...
                        <td className="py-1.5 pr-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                              {/* symbol + pills + allocation mini-bar (fill = current, tick = target, amber tick = reserve floor) */}
                              <div className="flex items-center gap-2">
                                ...
                                <div className="relative h-[3px] rounded-full bg-[#0e1c28] overflow-hidden flex-shrink-0" style={{ width: 130 }}>
                                  <div
                              ...
                              <div className="text-dim mt-0.5">
                                {truncate(token.mint, 4)}
                                {isLend && (
                                  <span className="text-cyan/80"> · ${lentValueUsd.toFixed(2)} lent · {lendApy.toFixed(2)}% APY</span>
                                )}
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="text-right py-1.5 text-muted tabular-nums">{h ? h.balance.toFixed(4) : "—"}</td>
                        <td className="text-right py-1.5 text-muted tabular-nums">{h ? formatSol(h.valueSol) : "—"}</td>
                        <td className="text-right py-1.5 text-muted tabular-nums">{h ? cur.toFixed(1) + "%" : "—"}</td>
                        <td className="text-right py-1.5 text-muted tabular-nums">{tgt.toFixed(1)}%</td>
