...
![Version](https://img.shields.io/badge/version-3.7.2-22d3ee) ![Node.js](https://img.shields.io/badge/Node.js-22-green) ![TypeScript](https://img.shields.io/badge/TypeScript-5-blue) ![Solana](https://img.shields.io/badge/Solana-mainnet-purple)
...
### v3.7.2
- **Portfolio hero polish** — the stat tiles now anchor to the bottom just above the meters (empty space rises to the top), and type is larger throughout (tile values, total, wallet balance, meter labels)

### v3.7.1
- **Portfolio hero stat strip** — filled the hero's empty middle with four secondary tiles: value in SOL, SOL-denominated P&L, Jupiter Lend position (lent $ · APY), and token count

### v3.7.0
- **Portfolio hero redesign** — the wallet balance now sits inline to the right of the total value (instead of a pinned bottom tile), and the ATH-ratio and peak-decay meters sit side by side in a single row rather than stacked

### v3.6.3
- **Settings layout** — the Lending (Jupiter Lend) card moved to the right column beneath Telegram / Daily Report, balancing the two columns (left keeps Wallet + Basket settings)

### v3.6.2
- **Compact Logs rows** — each rebalance/lending entry is now a single line (route, tx link, SOL value, cost/APY, time inline) instead of two, and the page size grew from 12 to 15 rows

### v3.6.1
- **Distribution legend** — the legend now top-aligns level with the DISTRIBUTION heading (donut stays centered) instead of hanging low, centered against the donut
